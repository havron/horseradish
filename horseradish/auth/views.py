...
from horseradish.constants import SUCCESS_METRIC_STATUS, FAILURE_METRIC_STATUS
from horseradish.extensions import metrics
from horseradish.common.utils import get_psuedo_random_string

from horseradish.users import service as user_service
from horseradish.roles import service as role_service
from horseradish.auth.service import (
    create_token,
    fetch_token_header,
    get_rsa_public_key,
)
from horseradish.auth import ldap
        ...
        for group in profile["googleGroups"]:
            ...
            if not role:
                role = role_service.create(
                    group,
                    description="This is a google group based role created by Horseradish",
                    third_party=True,
                )
...
def update_user(user, profile, roles):
    ...
    if not user:
        ...
        # we add 'horseradish' specific roles, so they do not get marked as removed
...
class Login(Resource):
    """
    Provides an endpoint for Horseradish's basic authentication. It takes a username and password
...
class Ping(Resource):
    """
    This class serves as an example of how one might implement an SSO provider for use with Horseradish. In
    this example we use an OpenIDConnect authentication flow, that is essentially OAuth2 underneath. If you have an
    OAuth2 provider you want to use Horseradish there would be two steps:
    ...
    2. Add or change the Horseradish AngularJS Configuration to point to your new provider
    """
